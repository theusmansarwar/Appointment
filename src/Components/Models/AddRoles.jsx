...
export default function AddRoles({
 ...
 const [errors, setErrors] = React.useState({});
  ...
  const allModules = ["Dashboard","User","Roles", "Appointment","PatientData","Reports","Records"];
  React.useEffect(() => {
...
try{
    ...
    console.log(":small_blue_diamond: Role Form Data:", roleData);
    let response;
    if (Modeltype === "Add") {
      // console.log(":pushpin: Adding new role...");
      response = await createRole(roleData);
    } else {
      console.log(":pushpin: Updating role with ID:", id);
      response = await updateRole(id, roleData);
    }
    // :white_check_mark: Alert show karna
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Box sx={{ display: "flex", gap: 2, mt: 2 }}>
            <FormControl fullWidth>
             ...
             {errors.modules && (
            <FormHelperText sx={{ color: "#B22222" }}>
              {errors.modules}
            </FormHelperText>
          )}
          </Box>
