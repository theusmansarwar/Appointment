...
export default function AddUsers({
  ...
  // :white_check_mark: Sync Modeldata on edit
  React.useEffect(() => {
    setId(Modeldata?._id || "");
  ...
  // :white_check_mark: Reset form after adding new user
  const resetForm = () => {
    setName("");
    setEmail("");
    setPassword("");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response?.status === 200 || response?.status === 201) {
        ...
        setErrors({});
        // :white_check_mark: Reset form only on "Create"
        if (!id) resetForm();
        handleClose();
